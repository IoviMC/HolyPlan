		...
		<div id="lat_derecho">
			...
			<table>
				...
				<c:forEach items="${pagos}" var="pago">
					<tr class="datos">
						...
						<td>${pago.importe} &euro;</td>
						<c:choose>
			    		<c:when test="${pago.pagoGastoTotal and pago.nombreUsuario eq nombreUsuario}">					
							<td><a href="${pageContext.request.contextPath}/historico/cobrarGastoColectivo/${pago.id}">Cobrar deuda</a></td>
						</c:when>
						<c:otherwise>
							<td></td>
						</c:otherwise>
			...
			<table>
				...
				<c:forEach items="${pagoTotalPorUsuario}" var="pagoTotal">
					<tr class="datos">
						<td>${pagoTotal.key}</td>
						<td>${pagoTotal.value} &euro;</td>
						<td>${gastoTotalUsuario} &euro;</td>						
						<td>${pagoTotal.value - gastoTotalUsuario} &euro;</td>						
					</tr>
				</c:forEach>
			</table>
			<c:if test="${idRol eq 1}">
				<p><a href="${pageContext.request.contextPath}/historico/saldarCuenta"><img src="<c:url value="/resources/img/money.png"/>" /> Saldar cuenta</a></p>
			</c:if>
